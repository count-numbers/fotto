  ...
  def parseCLIArgs(args: Array[String]): Map[Symbol, Any] = {
    ...
    def parseOptions(map: OptionMap, args: List[String]): OptionMap = {
      args match {
        ...
        case "--low-resolution" :: lowRes:: tail => parseOptions(map ++ Map('lowResolution-> lowRes.toBoolean), tail)
        case "--view" :: view :: tail => parseOptions(map ++ Map('view -> view.toBoolean), tail)
        case "--crop-marks" :: cropMarks :: tail => parseOptions(map ++ Map('cropMarks-> cropMarks.toBoolean), tail)
